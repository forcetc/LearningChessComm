...
public class JSONFactory 
{
	...
	public static String createJSONFromJSONState(JSONState jsonState)
	{
		...
		try 
		{
			...
			for (PieceState pieceState : jsonState.getPieceStates()) 
			{
				if(pieceState != null)
				{
			...
			if(jsonState.isPawnMovedTwo() && jsonState.getPawnMovedTwoLocation() != null)
			{
				JSONArray pieceJumpedLocation = new JSONArray();
				pieceJumpedLocation.put(0,jsonState.getPawnMovedTwoLocation().getColumn());
				pieceJumpedLocation.put(1,jsonState.getPawnMovedTwoLocation().getRow());
			...
			jsonStateObject.put("pawnJumped", jsonState.isPawnMovedTwo());
	...
	private static JSONObject generateJSONFromPieceStateObject(PieceState pieceState)
	{
		...
		if(pieceState != null)
		{
			...
			{
				throw new IllegalArgumentException("There was an error in generating JSON from the PieceState object. The pieceState name \"" + pieceName + "\" did not match any of the options.");
			}
		}
...
//			Set up the JSON location array values (Column, then Row)
			JSONArray pieceStateLocationArray = generateJSONLocationArrayFromIntValues(pieceState.getLocation().getColumn(), pieceState.getLocation().getRow());
			
			//Create the PieceStateObject with the created pieces.
			pieceStateObject = new JSONObject();
			pieceStateObject.put("name", pieceState.getPieceName());
...
//			Set up the JSON location array values (Column, then Row)
			...
			pieceStateObject = new JSONObject();
			pieceStateObject.put("name", pieceState.getPieceName());
			pieceStateObject.put(getSpecialBooleanType(name), pieceState.isSpecialMoveInformation());
			pieceStateObject.put("location", pieceStateLocationArray);
			pieceStateObject.put("isWhite", pieceState.isWhite());
		}
		catch(JSONException e)
